...
const CheckItem = ({ text }: { text: string }) => (
  <li className="flex items-center gap-3 text-sm text-foreground">
    <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center text-primary text-xs font-bold">✓</span>
    {text}
  </li>
);
...
const AppShowcase = () => {
  ...
  return (
    <section ref={sectionRef} className="py-28 md:py-36 bg-warm-cream relative overflow-hidden">
      <div className="absolute top-20 -right-20 w-96 h-96 rounded-full border border-primary/5" />
      <div className="absolute bottom-20 -left-20 w-72 h-72 rounded-full border border-gold/5" />

      <div className="container px-4">
        <motion.div className="text-center mb-20" variants={blurIn} initial="hidden" whileInView="visible" viewport={vp}>
          <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary font-semibold text-xs tracking-[0.2em] uppercase mb-4">L'application</span>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-display font-bold text-foreground mt-3 mb-5">Conçue pour le terrain</h2>
